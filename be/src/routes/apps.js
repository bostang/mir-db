...
// --- RUTE READ (Bisa diakses oleh admin & viewer) ---
...
// 2. DOWNLOAD CSV
router.get('/download/csv', verifyToken, (req, res, next) => {
    const query = "SELECT * FROM apps";
    db.query(query, (err, rows) => {
        if (err) return next(err);
...
router.get('/:id/people', verifyToken, (req, res, next) => {
    const applicationId = req.params.id;
    const query = `
...
// --- RUTE WRITE (Hanya bisa diakses oleh ADMIN) ---
...
router.put('/:id', verifyToken, isAdmin, (req, res, next) => {
    ...
    db.query(query, [nama_aplikasi, deskripsi_aplikasi, business_owner, applicationId], (err, result) => {
        if (err) return next(err);
        // Note: msnodesqlv8 mengembalikan rowsAffected sebagai array atau property tergantung versi, 
        // pastikan pengecekan ini sesuai dengan perilaku library Anda.
